<template>
  ...
  >
    ...
    <!-- <el-button slot="reference" size='small'
        style='width:200px;text-align:left;padding:0px;border:none'> -->
    <el-input
      v-if="!option.labelType"
      slot="reference"
      @input="changeInput"
      v-bind:value="inputs[option.param]"
      :size="size"
      :placeholder="
        option.placeholder
          ? option.placeholder
          : nt('publicCommon_main_label_pSelect') + option.label
      "
    ...
    <el-input
      ...
      slot="reference"
      style="width: 200px"
      :placeholder="nt('customRetrieve_index_info_pleaseSelectTemplate')"
      v-model="inputs[option.param]"
    >
    ...
    <!-- <el-button
      ...
      >
        {{inputs[option.param] ||
          nt('customRetrieve_index_info_pleaseSelectTemplate')}}
...
export default {
  ...
  watch: {
    ...
    tableData(val, oldVal) {
      ...
      // 判断是否有回显
      if (this.inputs[this.option.param]) {
        const values = this.inputs[this.option.param].split(',');
        // console.log(values);
        val.forEach((item) => {
  ...
  methods: {
    ...
    changeInput(val) {
      if (this.option.notInput) {
        // 对于不能输入的字段
        this.inputs[this.option.param] = this.inputs[this.option.param];
      } else {
        this.inputs[this.option.param] = val;
    ...
    handleCurrentChange(val) {
      if (this.option.single && val) {
        ...
        this.inputs[this.option.param] = val.label;
        this.inputs[this.option.paramKey] = val.value
          ? val.value.toString()
          : val.value;
    ...
    clearInput() {
      this.$refs.pageSelTable.clearSelection();
      this.inputs[this.option.param] = ''; // todo 全清还是只清当前页
      if (Array.isArray(this.inputs[this.option.paramKey])) {
        // 如果是数组设置成空数组
        this.inputs[this.option.paramKey] = [];
      } else {
        this.inputs[this.option.paramKey] = '';
      }
    ...
    addRow(row) {
      if (this.option.paramKey) {
        ...
        if (Array.isArray(this.inputs[this.option.paramKey])) {
          // 判断类型
          this.inputs[this.option.paramKey].push(row.value);
        } else if (typeof this.inputs[this.option.paramKey] == 'string') {
          this.inputs[this.option.paramKey] = row.value;
        }
      }
      this.inputs[this.option.param] += `,${row.label}`;
      // 去掉字符串后的最后一个逗号
      const reg = /^,/gi;
      this.inputs[this.option.param] = this.inputs[this.option.param].replace(
        reg,
        ''
      );
    },
    ...
    deleteRow(row) {
      if (this.option.paramKey) {
        const arr = this.inputs[this.option.paramKey];
        // this.inputs[this.option.paramKey].remove(row.value);
      ...
      const selectedParamList = this.inputs[this.option.param].split(',');
      const curIndex = selectedParamList.findIndex((item) => item == row.label); // 查找在选中列表的第几个
      if (curIndex != -1) {
        // 表示能找到
        selectedParamList.splice(curIndex, 1);
      }
      this.inputs[this.option.param] = selectedParamList.join(',');
    },
    /**
     * 切换单行选中
     */
    ...
    handleSelectionChange(val) {
      // console.log(val, this.inputs[this.option.param], 'select');
      this.selectList = val;
    }
  },
  created() {
